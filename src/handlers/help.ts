...
export function registerHelpHandler(bot: Bot<BotContext>): void {
  bot.command("help", async (ctx) => {
    ...
    if (isPrivate) {
      const lines = [
        ctx.t("help_private_title"),
        "",
        ctx.t("help_section_personal"),
        "<code>/settings</code> - Interactive dashboard (Rate, Address, Remark, Granularity, Language)",
        "<code>/export</code> - Download your work history as CSV",
        "",
        ctx.t("help_section_group"),
        "<code>/stats [#tag]</code> - View your weekly/monthly work stats",
        "<code>/history</code> - Unified activity timeline",
        "<code>/team</code> - Group admin dashboard",
